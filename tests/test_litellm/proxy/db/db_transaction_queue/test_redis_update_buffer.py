...
async def test_store_in_memory_spend_updates_uses_pipeline(
    ...
    # Create mock queues - only 3 queue families have data
    ...
    daily_project_queue.flush_and_get_aggregated_daily_spend_update_transactions = (
        AsyncMock(return_value={})
    )
...
async def test_get_all_transactions_from_redis_buffer_pipeline(
    ...
    """
    # Simulate pipeline results: slot 0 = spend updates, remaining slots = daily categories
    db_spend_json = json.dumps(
        {
            ...
            "project_list_transactions": {"project1": 3.0},
    ...
    daily_project_json = json.dumps(
        {"project_key1": {"spend": 3.0, "api_requests": 3}}
    )
    ...
    mock_redis_cache.async_lpop_pipeline = AsyncMock(
        return_value=[
            ...
            [daily_project_json],  # slot 4: daily project
    ...
    assert db_spend["project_list_transactions"]["project1"] == 3.0
    ...
    assert daily_project is not None
    assert daily_project["project_key1"]["spend"] == 3.0
