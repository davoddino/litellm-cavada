...
function MetricsSection({ logEntry, metadata }: { logEntry: LogEntry; metadata: Record<string, any> }) {
  ...
  const companyId = logEntry.company_id ?? logEntry.organization_id ?? metadata?.user_api_key_org_id;
  const projectId = logEntry.project_id ?? metadata?.user_api_key_project_id;
  const companyDisplay = logEntry.company_name ?? companyId;
  const projectDisplay = logEntry.project_name ?? projectId;
    ...
    <div className="bg-white rounded-lg shadow w-full max-w-full overflow-hidden mb-6">
      <Card title="Metrics" size="small" style={{ marginBottom: 0 }}>
        <Descriptions column={2} size="small">
          ...
          {companyDisplay && <Descriptions.Item label="Company">{companyDisplay}</Descriptions.Item>}
          {projectDisplay && <Descriptions.Item label="Project">{projectDisplay}</Descriptions.Item>}
